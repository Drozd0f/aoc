...
from year2023.day2.part_1 import Cubes, SET_NUMBER_REGEX
...
class Part2:
    ...
    def calculate(self, file_name: str) -> int:
        ...
        return self.power_games

    def read_raw_game(self, line: str):
        game_set = Cubes()
        for raw_cub in re.finditer(SET_NUMBER_REGEX, line):
            if raw_cub[1] == "blue":
                game_set.blue_cube = max(game_set.blue_cube, int(raw_cub[0]))
            elif raw_cub[1] == "red":
                game_set.red_cube = max(game_set.red_cube, int(raw_cub[0]))
            elif raw_cub[1] == "green":
                game_set.green_cube = max(game_set.green_cube, int(raw_cub[0]))

        self.power_games += game_set.red_cube * game_set.green_cube * game_set.blue_cube
